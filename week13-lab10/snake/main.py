...
cur.execute("SELECT * FROM snakegame WHERE username = %s", (NICKNAME,))
result = cur.fetchone()
if result is None:
    save_game_state(NICKNAME, 0, 0)
    conn.commit()
else:
    print("User's max score:{}".format(result[1]))
    print("User's max level:{}".format(result[2]))
...
def generate_food(walls):
    while True:
        food_x = round(random.randrange(size_of_snake_block, screen_width - size_of_snake_block) / 10.0) * 10.0
        food_y = round(random.randrange(40, screen_height - size_of_snake_block) / 10.0) * 10.0
        collision = False
        for wall in walls:
            if wall.x <= food_x < wall.x + wall.width and wall.y <= food_y < wall.y + wall.height:
                collision = True
                break
        if not collision:
